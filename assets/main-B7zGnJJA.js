import { g as gsapWithCSS, S as Scene, A as AmbientLight, D as DirectionalLight, P as PerspectiveCamera, W as WebGLRenderer, B as BufferGeometry, a as BufferAttribute, b as PointsMaterial, c as Points, G as Group, d as SphereGeometry, M as MeshBasicMaterial, e as Mesh, L as LineBasicMaterial, f as Line, C as Clock } from "./vendor-BUcLmP5o.js";
(function polyfill() {
...
function createHeader() {
  ...
  allNavLinks.forEach((link) => {
    ...
    if (href === "/" || href === "/index.html" || (href == null ? void 0 : href.endsWith("/index.html"))) {
    ...
    } else if (!(href == null ? void 0 : href.includes("#"))) {
...
const coursesData = {
  ...
  getFeaturedCourses: function(level, limit = 3) {
    ...
    return this[level].filter((course) => course.featured).slice(0, limit);
  },
  /**
...
function init() {
  ...
  const ambientLight = new AmbientLight(16777215, 0.15);
  scene.add(ambientLight);
  const directionalLight = new DirectionalLight(16777215, 0.3);
  ...
  const camera = new PerspectiveCamera(28, sizes.width / sizes.height, 0.1, 100);
  ...
  renderer.setSize(sizes.width, sizes.height);
  renderer.setPixelRatio(Math.min(window.devicePixelRatio, 2));
  renderer.setClearColor(328965, 0);
...
function createNeuralNetwork() {
  ...
  for (let i = 0; i < nodeCount; i++) {
    ...
    const nodeGeometry = new SphereGeometry(nodeSize, 8, 8);
...
function setupSectionAnimations() {
  ...
  function animateHeroSection(section) {
    ...
    gsapWithCSS.killTweensOf(heroContent);
    gsapWithCSS.killTweensOf(heroContent.children);
    heroContent.style.visibility = "visible";
    gsapWithCSS.set(heroContent, {
      opacity: 0,
      y: 30
    });
    gsapWithCSS.to(heroContent, {
      opacity: 1,
      y: 0,
      duration: 1.2,
      ease: "power3.out",
